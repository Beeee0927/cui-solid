...
export default function TeleportBoxPage() {
  const data: any[] = new Array(30)
    .fill(0)
    .map((_, index) => ({ value: index, label: `列表选项${index}` }))
        ...
        <InnerCheckbox
          ...
          label={
            <Space align="center" style={{ height: '46px' }}>
              <Avatar
                style={{ 'background-color': 'var(--cui-theme-color-gold)' }}
              >
                {(item.label as string).substring(0, 1)}
              </Avatar>
              <Space
                dir="v"
                size={0}
                style={{ 'min-width': 0, 'line-height': '18px' }}
              >
  ...
  const renderSelectedItem = (item: TeleportBoxItem, onRemove: () => void) => {
    return (
      <BothSide class="cm-teleprt-custom-select-item">
        <Space align="center" style={{ height: '46px' }}>
          <Avatar style={{ 'background-color': 'var(--cui-theme-color-gold)' }}>
            {(item.label as string).substring(0, 1)}
          </Avatar>
          <Space
            dir="v"
