...
export const Col = (props: ColProps) => {
  ...
  responsiveTypes.forEach((type: string) => {
    if (props[type as keyof ColProps]) {
      const w =
        typeof props[type as keyof ColProps] === 'number'
          ? props[type as keyof ColProps]
          : props[type as keyof ColProps].grid
      const respClass: string = createGrid(w, type as responsiveType)
      if (respClass) {
        respClassList[respClass] = true
      }
      const offset =
        typeof props[type as keyof ColProps] === 'object'
          ? props[type as keyof ColProps].offset
          : 0
  ...
  const classList = () =>
    useClassList(props, 'cm-col', {
      ...respClassList,
      ...offsetClassList
    })
  return (
    <div ref={ref} classList={classList()} style={style()}>
      {props.children}
    </div>
  )
}
