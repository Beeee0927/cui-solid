...
export function Pagination(props: PaginationProps) {
  ...
  function _getInner() {
    ...
    const start =
      current() > startEndShowNum + innerNear + 1
        ? current() - innerNear
        : startEndShowNum + 1
    const end =
      current() + innerNear + startEndShowNum >= pages
        ? pages - startEndShowNum
        : current() + innerNear
    return { start, end }
  }
  ...
  function rednderItems() {
    ...
    for (let i = 1; i <= startEndShowNum; i++) {
      ...
      pagerList.push(
        <PageItem
          active={active}
          onClick={_handleChange.bind(null, i)}
          currentIndex={i}
        />
      )
    }
    ...
    if (cur > startEndShowNum + innerNear + 1) {
      pagerList.push(
        <li class="cm-pagination-num cm-pagination-ellipse">
    ...
    for (; start <= end; start++) {
      ...
      pagerList.push(
        <PageItem
          onClick={_handleChange.bind(null, start)}
          currentIndex={start}
          active={active}
        />
      )
      ...
      <Switch>
        <Match when={props.mini}>
          <ul class="cm-pagination-num-list">
            ...
            <Input
              style={{ width: props.size === 'small' ? '35px' : '50px' }}
              class="mr-5"
              value={[pageNum, setPageNum]}
              size={props.size}
              onChange={gotoPage}
            />
