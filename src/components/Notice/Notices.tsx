...
function NoticePanel(props: any) {
  ...
  const ic = () =>
    icon === undefined
      ? icons[theme!]
        ? createComponent(icons[theme!], { class: `cm-notice-icon-${theme}` })
        : null
      : icon
  ...
  return (
    <div classList={classList()} style={style} ref={wrap}>
      <div class="cm-notification-item-wrap">
        <Show when={hasIcon()}>
          <div class="cm-notification-icon">{ic()}</div>
        </Show>
        <div class="cm-notification-content">
          ...
          <Show when={btn}>
            <span class="cm-notification-btn-wrap">{btn}</span>
          </Show>
        </div>
      </div>
    </div>
...
export function NoticeBox(props: NoticesProps): JSX.Element {
  ...
  return (
    <Show when={data() && data().length}>
      <div
        class={`cm-notification-box cm-notification-${props.docker}`}
