...
export interface TreeContextProps {
  ...
  onDrop: (
    e: any,
    targetKey: NodeKeyType,
    dropPosition: dragHoverPartEnum
  ) => void
...
export function Tree(props: TreeProps) {
  ...
  const onNodeExpandCollapse = async (node: TreeNode) => {
    const valid = props.beforeExpand
      ? await props.beforeExpand(node, !node.expand)
      : true
    if (!valid) {
      return
    }
    ...
    props.ref({
      ...
      scrollTo: (
        ...
        if (position === 'center') {
          top =
            top -
            vir.getScrollElement().getBoundingClientRect().height / 2 +
            nodeHeight / 2
        }
        if (position === 'bottom') {
          top =
            top -
            vir.getScrollElement().getBoundingClientRect().height +
            nodeHeight
        }
    ...
    if (e.dataTransfer) {
      try {
        ...
        const shouldDrop: boolean = props.beforeDropMethod
          ? await props.beforeDropMethod(node, dragNode, hoverPart)
          : true
      ...
      <Show
        ...
        fallback={
          <div class="cm-tree" style={style()}>
            <Show
              when={props.data && props.data.length}
              fallback={<div class="cm-tree-empty">{emptyText}</div>}
            >
              <VirtualList
                ref={vir}
                items={store.nodeList}
                itemEstimatedSize={22}
                itemComponent={{ component: NodeWrap, props: {} }}
              />
            </Show>
          </div>
        }
