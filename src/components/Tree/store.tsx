...
export class TreeStore {
  ...
  constructor(props: TreeProps) {
    ...
    const [selected, setSelected] = createModel<NodeKeyType>(
      props,
      'selected',
      ''
    )
  ...
  storeNode = (node: TreeNode) => {
    ...
    this.setStore(
      'nodeMap',
      produce((sourceMap: Record<NodeKeyType, TreeNode>) => {
    ...
    if (node) {
      ...
      if (parent) {
        const index: number = parent.children!.findIndex(
          (item: TreeNode) => item[this.keyField] === node[this.keyField]
        )
        this.setStore(
          'nodeMap',
          parent[this.keyField],
          produce((target: TreeNode) => {
  ...
  append = (parentKey: any, nodeId: NodeKeyType | TreeNode) => {
    ...
    this.setStore(
      'nodeMap',
      produce((map: Record<NodeKeyType, TreeNode>) => {
    ...
    this.setStore(
      ...
      produce((map: Record<NodeKeyType, TreeNode>) => {
        ...
        const targetIndex = parent.children!.findIndex(
          (item) => item[this.keyField] === targetKey
        )
  ...
  getNodeIndexInShow = (nodeId: TreeNode | NodeKeyType) => {
    ...
    const index = this.store.nodeList.findIndex(
      (n: TreeNode) => n[this.keyField] === node[this.keyField]
    )
  ...
  setCheckedForwardUp = (node: TreeNode) => {
    ...
    if (parentNode) {
      const checked: boolean | 'indeterminate' | undefined =
        this.getNodeChecked(parentNode)
      this.setStore(
        'nodeMap',
        parentNode[this.keyField],
        produce((n: TreeNode) => {
          n.checked = checked
        })
      )
  ...
  loadData = async (
    node: TreeNode,
    loadDataMethod: (node: TreeNode) => Promise<TreeNode[]>
  ) => {
    this.setStore(
      'nodeMap',
      node[this.keyField],
      produce((n: TreeNode) => (n.__loading = true))
    )
    try {
      ...
      this.setStore(
        'nodeMap',
        node[this.keyField],
        produce((n: TreeNode) => (n.__loading = false))
      )
    }
    this.setStore(
      'nodeMap',
      node[this.keyField],
      produce((n: TreeNode) => (n.loading = false))
    )
  }
   ...
   */
  getChildChecked = () => {
    return this.flatData.filter(
      (node: TreeNode) =>
        node.checked === true && (!node.children || node.children.length === 0)
    )
  }
  ...
  getHalfChecked = () => {
    return this.flatData.filter(
      (node: TreeNode) =>
        node.checked === true || node.checked === 'indeterminate'
    )
  }
