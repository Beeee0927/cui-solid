...
export function TreeSelect(props: TreeSelectProps) {
  const [value, setValue] = createField<NodeKeyType | NodeKeyType[]>(
    props,
    props.multi ? [] : ''
  )
  ...
  createEffect(() => {
    ...
    if (
      props.multi &&
      (outValue as NodeKeyType[]).join(',') === getChecked().join(',')
    ) {
      return
    }
  })
  ...
  return (
    <div classList={classList()} style={props.style} tabIndex="1">
      <Dropdown
        ...
        menu={
          <div class="cm-tree-select-wrap">
            <Tree
              ...
              ref={tree}
              value={props.multi ? ([value, setValue] as any[]) : []}
              selected={props.multi ? '' : ([value, setValue] as any)}
