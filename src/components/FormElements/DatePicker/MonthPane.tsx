...
export function MonthPane(props: any) {
  ...
  const year = () => {
    if (
      ...
      return (
        store.currentMonth[index] &&
        store.currentMonth[index].getFullYear &&
        store.currentMonth[index].getFullYear()
      )
    } else {
  ...
  const month = () => {
    if (
      ...
      return (
        store.currentMonth[index] &&
        store.currentMonth[index].getMonth &&
        store.currentMonth[index].getMonth() + 1
      )
    } else {
  ...
  const onSelect = (type: string, num: number) => {
    ...
    setStore(
      'currentMonth',
      props.name === 'end'
        ? [store.currentMonth[0], d]
        : [d, store.currentMonth[1]]
    )
    // dateRange的时候不需要触发选择
      ...
      >
        <div class="cm-date-picker-month-header">
          <Button
            ...
            theme="borderless"
            onClick={onBack}
            icon={<FeatherChevronLeft size={16} />}
          >
            返回选择日期
          </Button>
        </div>
