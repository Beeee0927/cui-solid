...
export function Datepicker(props: DatepickerProps) {
  ...
  const [value, setValue] = createField<any>(
    props,
    'value',
  ...
  const classList = () =>
    useClassList(props, 'cm-date-picker', {
      ...
      'cm-date-picker-clearable':
        !props.disabled && props.clearable && value() && value().length !== 0
    })
  ...
  const onSelectDate = (day: Date, name: string) => {
    ...
    if (type === 'dateTime' || type === 'dateTimeRange') {
      ...
      if (type === 'dateTimeRange') {
        val =
          val && val.length
            ? val[store.range.length === 1 ? 1 : 0]
            : store.currentMonth[store.range.length === 1 ? 1 : 0]
      } else {
    ...
    const origin =
      v() ||
      (type === 'monthRange' || type === 'dateRange' || type === 'dateTimeRange'
        ? [now, now]
        : now)
    ...
    >
      <div classList={classList()} style={props.style}>
        <Dropdown
          ...
          menu={
            <div class="cm-date-picker-wrap">
              <Show when={props.shortCuts}>
                <div class="cm-date-picker-shortcuts">
                  {typeof props.shortCuts === 'function'
                    ? props.shortCuts()
                    : props.shortCuts}
                </div>
              </Show>
        ...
        >
          <Show
            when={!props.trigger}
            fallback={props.trigger && props.trigger()}
          >
            <Value
