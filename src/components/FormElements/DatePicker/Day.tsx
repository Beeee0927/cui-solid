...
export function Day(props: any) {
  ...
  const isToday = props.day
    ? today.toLocaleDateString() === props.day.toLocaleDateString()
    : false
  const isActive = () =>
  ...
  const inRange = () =>
    props.range && props.day ? isBetween(props.range, props.day) : false
  const firstRange = () =>
    props.range &&
    props.range[0] &&
    props.day &&
    sameDate(props.range[0], props.day)
  const lastRange = () =>
    props.range &&
    props.range[1] &&
    props.day &&
    sameDate(props.range[1], props.day)
  const inHover = () => {
    const hoverDate =
      props.range && props.range.length === 1 && props.hoverDate
        ? [props.hoverDate, props.range[0]]
        : []
    hoverDate.length === 2 &&
      hoverDate.sort((a, b) => {
  ...
  return (
    <div classList={classList()} onClick={onDayClick} onMouseOver={onMouseOver}>
