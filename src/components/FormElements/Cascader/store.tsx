...
export class CascaderStore {
        ...
        keys.forEach((key: any) => {
          if (this.valMap[key]) {
            ...
            if (item && item.children) {
              const levelIds = item.children.map(
                (aItem: any) => aItem[this.valueField]
              )
  ...
  filter(keyword: string) {
    if (keyword) {
      const allChildren = this.flatData.filter(
  ...
  checkAll = () => {
    this.setStore(
      'nodeMap',
      produce((map: Record<NodeKeyType, CascaderNode>) => {
  ...
  loadData = async (
    node: CascaderNode,
    loadDataMethod: (node: CascaderNode) => Promise<CascaderNode[]>
  ) => {
    try {
    ...
    this.setStore(
      'nodeMap',
      node[this.valueField],
      produce((n: CascaderNode) => (n.loading = false))
    )
  }
