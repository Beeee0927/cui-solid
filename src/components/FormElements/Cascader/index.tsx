    ...
    >
      <div classList={classList()} tabIndex="0">
                    ...
                    <For each={store.store.columns}>
                      {(column: any[], index: Accessor<number>) => {
                        return (
                          <Menu
                            key={
                              store.store.selectedKey()[index() - 1] || 'root'
                            }
                            data={column}
                            trigger={trigger}
                ...
                <div class="cm-cascader-wrap" ref={filterWrap}>
                  <div
                    classList={{
                      ...
                      'cm-cascader-list-empty':
                        !store.store.filteredList?.length
                    }}
                  >
                    <Show
                      when={store.store.filteredList?.length}
                      fallback={
        ...
        >
          ...
          >
            <span class="cm-cascader-trigger">
              {props.triggerRender?.(text(), store.value())}
            </span>
          </Show>
        </Dropdown>
      </div>
