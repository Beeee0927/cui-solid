...
export function InnerInput(props: InputProps) {
  ...
  return (
    <div classList={clazzName()} style={local.style}>
      {local.prefix ? (
        <div class="cm-input-prefix" style={local.prefixStyle}>
          {local.prefix}
        </div>
      ) : null}
      {local.prepend ? (
      ...
      {local.clearable && _value() ? (
        <Icon class="cm-input-clear" name="x-circle" onClick={clear} />
      ) : null}
      {local.suffix || (props.wordCount && props.maxLength) ? (
        <div class="cm-input-suffix" style={local.suffixStyle}>
          <Show
            when={props.wordCount && props.maxLength}
