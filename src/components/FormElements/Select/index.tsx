...
export function Select(props: SelectOptions) {
  ...
  const classList = () =>
    useClassList(props, 'cm-select', {
      ...
      'cm-select-clearable':
        !props.disabled && props.clearable && `${value()}`.length !== 0,
      ...
      return arr.length
        ...
        : props.emptyOption
          ? [
              {
                id: '',
                title: props.renderSelectedItem?.(null) || props.emptyOption
              }
            ]
          : []
    } else {
      return arr.length
        ? props.renderSelectedItem?.(arr[0].data) || arr[0].title
        : props.emptyOption
          ? props.emptyOption
          : ''
    }
  ...
  return (
    <div classList={classList()} style={props.style} ref={wrap}>
        ...
        menu={
          <div class="cm-select-options-wrap">
            ...
            >
              <Show
                when={!props.loading}
                fallback={<div class="cm-select-loading">加载中</div>}
              >
                <ul class="cm-select-option-list" ref={filterWrap}>
                  <Show
                    when={displayItems().length}
                    fallback={
                      <Exception
                        width={100}
                        type="empty"
                        typeImage={NO_DATA_IMAGE}
                        desc={props.emptyText}
                      />
                    }
                  >
                    <VirtualList
      ...
      >
        <Show
          ...
          fallback={
            <Value
              ...
              size={props.size}
              icon={<FeatherChevronDown class="cm-select-cert" />}
        ...
        >
          <span class="cm-select-trigger">
            {props.triggerRender?.(labels())}
          </span>
        </Show>
      </Dropdown>
    </div>
    ...
    <Show
      ...
      fallback={
        <Option
          ...
          onClick={(v: any) => props.onOptionClick(v, item)}
        />
      }
    >
      <EmptyOption
        visible
        data={{ label: item[props.textField], value: '' }}
        checked={props.value() === ''}
