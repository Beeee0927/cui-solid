...
export function Anchor(props: AnchorProps) {
  ...
  const handleScrollTo = () => {
    ...
    const currentLinkElementA: any = document.querySelector(
      `a[data-href="${store.currentLink}"]`
    )
  ...
  const getContainer = () => {
    scrollContainer = local.container
      ? typeof local.container === 'string'
        ? document.querySelector(local.container)
        : local.container
      : window
    scrollElement = local.container
      ? scrollContainer
  ...
  const gotoAnchor = (href: any, e: any) => {
    ...
    if (mode === 'hash') {
      ...
      const hash =
        index > -1 ? location.hash.substring(0, index) : location.hash
      const params = new URLSearchParams(search)
      params.set('_to', href)
      window.history.replaceState(
        {},
        '',
        `${location.pathname}${hash}?${params.toString()}`
      )
    }
  }
            ...
            return (
              <div class="cm-anchor-link">
                <a
                  ...
                  href={link.href}
                  data-scroll-offset={local.scrollOffset || 0}
                  data-href={link.href}
                  onClick={(e: any) => {
