...
export function Tabs(props: TabsProps) {
  ...
  // 监听activeName的变化
  createEffect(() => setActiveName(props.activeName || ''))
  ...
  const scrollPrev = () => {
    ...
    const scrollProp =
      align === 'top' || align === 'bottom' ? 'scrollLeft' : 'scrollTop'
  ...
  const observeIntersection = () => {
    const ob = new IntersectionObserver(
      (entries) => {
        ...
        const firstDisplayIndex = store.tabs.findIndex(
          (item) => item.name === firstDisplayItemName
        )!
            ...
            return (
              <DropdownItem data={item} name={item.name} icon={item.icon}>
                ...
                <Show when={item.closeable}>
                  <FeatherX
                    onClick={onRemove.bind(null, item.name)}
                    size={12}
                  />
                </Show>
              </DropdownItem>
    ...
    <div classList={classList()} style={props.style}>
      <div class="cm-tabs-header-wrap">
        <Show when={props.prepend}>
          <div class="cm-tabs-extra-prepend">{props.prepend}</div>
        </Show>
        <Show
          when={
            props.arrowPosition === 'start' || props.arrowPosition === 'both'
          }
        >
          <Dropdown
            disabled={!props.more}
            align={prevDropdownAlign()}
          ...
          >
            ...
            >
              <Show
                when={align === 'top' || align === 'bottom'}
                fallback={<FeatherChevronUp size={16} />}
              >
        ...
        <Show when={props.arrowPosition === 'start'}>
          ...
          >
            ...
            >
              <Show
                when={align === 'top' || align === 'bottom'}
                fallback={<FeatherChevronDown size={16} />}
              >
        ...
        <Dropdown
          ...
          align="bottom"
          onSelect={props.onSelectContextMenu}
          {...props.dropdownProps}
        >
          <div
            class="cm-tabs-scroll"
            ref={scroll}
            onScroll={onScroll}
              ...
              <For each={store.tabs}>
                {(item: TabPaneProps) => {
                  const className = () => ({
                    ...
                    [`cm-tabs-header-item-${props.buttonTheme}`]:
                      !!props.buttonTheme,
                  ...
                  const el = () => (
                    <li
                      ...
                      onContextMenu={props.onContextMenu?.bind(null, {
                        ...item
                      })}
                    >
                      <div>
                        {item.icon}
                        <span
                          class="cm-tabs-header-item-text"
                          style={{ 'max-width': `${props.maxTabSize}px` }}
                        >
                          {item.title}
                        </span>
                        <Show when={item.closeable}>
        ...
        <Show
          when={
            store.scroll &&
            props.more &&
            store.moreList.length &&
            !props.arrowPosition
          }
        >
          <Dropdown
            align={moreDropdownAlign()}
            {...props.moreDropdownProps}
            onSelect={onSelectByName}
            class={'cm-tabs-more-dropdown'}
            menu={
              <DropdownMenu>
                <For each={store.moreList}>
                  {(item: TabPaneProps) => {
                    return (
                      <DropdownItem data={item} name={item.name}>
                        {item.title}
                        <Show when={item.closeable}>
                          <FeatherX
                            onClick={onRemove.bind(null, item.name)}
        ...
        <Show
          when={props.arrowPosition === 'end' || props.arrowPosition === 'both'}
        >
          <Dropdown
            disabled={!props.more}
            align={nextDropdownAlign()}
        ...
        <Show when={props.append}>
          <div class="cm-tabs-extra-append">{props.append}</div>
        </Show>
      </div>
      <div class="cm-tabs-content-wrap">
        <div class="cm-tabs-content" style={contextStyle()}>
          <For each={store.tabs}>
            {(item: TabPaneProps) => {
              const panelClass = () =>
                useClassList(item, 'cm-tab-panel', {
