...
function isColor(strColor: string | undefined): boolean {
  if (
    strColor &&
    (strColor.startsWith('#') ||
      strColor.startsWith('rgb') ||
      strColor.startsWith('hsl'))
  ) {
...
export function Badge(props: BadgeProps) {
  ...
  const showCount = () => {
    return props.count && props.count > overflowCount
      ? Math.min(overflowCount, props.count) + '+'
      : props.count
  }
    ...
    <span classList={classList()}>
      ...
      >
        <Show when={props.count !== undefined || props.text !== undefined}>
          <sup classList={countClass()} style={styles()}>
            {showCount()}
            {props.text}
          </sup>
        </Show>
