        ...
        <Match when={props.data._type !== 'expandChildren'}>
          <For each={props.store.columns}>
            {(column: ColumnProps, columnIndex: Accessor<number>) => {
              ...
              return (
                <Show when={rowSpan && colSpan} fallback={null}>
                  <Cell
                    type="td"
                    data={props.data}
                    column={column}
  ...
  createEffect(() => {
    ...
    if (props.virtual) {
      const scrollElHeight =
        props.height ?? document.documentElement.clientHeight
            ...
            <tbody>
              <For each={props.data.data}>
                {(rowData: any, index: Accessor<number>) => {
                  return (
                    <Row data={rowData} index={index()} store={props.data} />
                  )
                }}
              </For>
              <Show when={!props.data.data || !props.data.data.length}>
