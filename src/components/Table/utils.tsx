...
export function initColumns(columns: ColumnProps[]) {
  ...
  if (cols) {
    ...
    if (minFixedRight < Number.MAX_VALUE) {
      cols[minFixedRight]
        ? (cols[minFixedRight].fixedRightFirst = true)
        : void 0
    }
  }
 ...
 * 滚动条滚动后更新固定列的样式
 */
export function updateScrollFixed(
  maxFixedLeft: number,
    ...
    if (column.sortMethod && typeof column.sortMethod === 'function') {
      ...
      arr.sort((a: any, b: any) => {
        ...
        if (/^[0-9\.]+$/g.test(a[name])) {
          return (column.sortType === 'asc' ? 1 : -1) * (a[name] - b[name]) > 0
            ? 1
            : -1
        } else {
          return (
            (column.sortType === 'asc' ? 1 : -1) *
...
export function _buildTreeData(
  data: any[],
  target: any[],
  level: number,
  show: boolean,
 ...
 * @param row
 */
export function showHideChildren(
  setStore: SetStoreFunction<TableStore>,
  row: any
) {
  setStore(
    ...
    produce((item: any) => (item._showChildren = !item._showChildren))
  )
  setStore(
    'data',
    produce((data: any) => {
 ...
 * @param row
 */
export function addRemoveExpand(
  setStore: SetStoreFunction<TableStore>,
  column: ColumnProps,
  row: any
) {
...
export const onResizeEnd = (
  ...
  setStore(
    'columns',
    (col: ColumnProps) => col.id === store.resizeId,
    produce((col: ColumnProps) => {
      let w = col.width
        ? parseFloat(col.width) + (store.posX - store.startX)
        : undefined
      if (w && col.minWidth) {
  ...
  store.columns.find((col: ColumnProps, index: number) => {
    ...
    if (flag) {
      nextId = store.columns[index + 1]
        ? store.columns[index + 1].id
        : undefined
    }
    return flag
  })
...
export const observerSizeChange = useDebounce(
  (
    store: TableStore,
    setStore: SetStoreFunction<TableStore>,
    wrap: Element
    ...
    const fxiedWidthColumns = store.columns.filter(
      (col: ColumnProps) => col.width
    )
    const fixedWidth = fxiedWidthColumns.reduce(
      (pre: number, cur: ColumnProps) =>
        pre + (cur.width ? parseFloat(cur.width) : 0),
      0
    )
    setStore(
      'columns',
      produce((columns: ColumnProps[]) => {
        const dynamicWidthColumns = columns.filter(
          (col: ColumnProps) => !col.width
        )
        if (dynamicWidthColumns.length > 0) {
          ...
          dynamicWidthColumns
            .filter((col: ColumnProps) => col.minWidth || col.maxWidth)
          ...
          const remianColumns = dynamicWidthColumns.filter(
            (col: ColumnProps) => !(col.minWidth || col.maxWidth)
          )
 ...
 * @returns
 */
export const getFlatColumns = (
  columns: ColumnProps[],
  flatColumns: ColumnProps[] = [],
  parent?: ColumnProps
) => {
  columns.forEach((col: ColumnProps) => {
    col.id = col.id ?? createUniqueId()
