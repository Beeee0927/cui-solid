...
export function Cell(props: any) {
  ...
  const cellClassList = () => ({
    ...
    'cm-table-cell-fixed-right-first':
      col.fixedRightFirst && props.showFixedRight
  })
  ...
  const text = () => {
    ...
    if (props.type === 'td') {
      ...
      if (column.type === 'checkbox') {
        return (
          <InnerCheckbox checked={props.checkedAll} onChange={onHeadChecked} />
        )
      }
          ...
          <div class="cm-table-cell">
            <Show when={col.tree}>
              <span
                ...
                style={{ 'padding-left': `${props.data._level * 16}px` }}
              />
              <Show
                when={props.data.children && props.data.children.length}
                fallback={<span class="cm-table-tree-icon-empty" />}
              >
                {treeIcon()}
              </Show>
            </Show>
            <Show when={col.ellipsis || col.tooltip} fallback={text()}>
              <Show
                when={col.tooltip}
                fallback={<span class="cm-table-cell-ellipsis">{text()}</span>}
              >
                <Popover
                  arrow
                  align={col.tooltipAlign || 'top'}
                  color={col.tooltipTheme}
