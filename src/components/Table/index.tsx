...
export function Table(props: TableProps) {
  ...
  const [selectedRowKeys, setSelectedRowKeys] = props.selectedRowKeys
  ...
  // 传入的columns变化
  createEffect(() => {
    const { maxFixedLeft, minFixedRight, columnsRows, calcColumns } =
  ...
  const onSelectRow = (row: any) => {
    ...
    if (lastRow) {
      setStore(
        'data',
        (item) => item.id === lastRow.id,
        produce((item: any) => (item._highlight = false))
      )
    }
    setStore(
      'data',
      (item) => item.id === row.id,
      produce((item: any) => (item._highlight = true))
    )
  ...
  createEffect(() => {
    ...
    if (rowKeys && rowKeys.length > 0) {
      setStore(
        'data',
        (item) => rowKeys.includes(item.id) && !item._checked,
