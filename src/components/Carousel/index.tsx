...
export function Carousel(props: CarouselProps) {
  ...
  const [views, setViews] = createSignal<number[]>(
    itemsPerView !== 'auto'
      ? new Array(Math.ceil(store.data.length / itemsPerView)).fill(1)
      : []
  )
  const loop = () =>
    (effect() === 'slide' && itemsPerView === 1) ||
    effect() === 'card' ||
    effect() === 'fade'
    ...
    if (index === views().length) {
      ...
      untrack(() => {
        ...
        list.style.transform =
          dir() === 'h' ? `translateX(${offset}px)` : `translateY(${offset}px)`
      })
    }
    if (index === -1) {
      ...
      untrack(() => {
        ...
        list.style.transform =
          dir() === 'h'
            ? `translateX(${-(views().length + 1) * w + offset}px)`
            : `translateY(${-(views().length + 1) * w + offset}px)`
      })
    }
    untrack(() => {
      transToActive()
  ...
  createEffect(() => {
    store.unActiveIndex >= 0 && (effect() === 'fade' || effect() === 'card')
      ? setStore('unActiveKey', store.data[store.unActiveIndex].id)
      : false
  })
  ...
  createEffect(() => {
    ...
    batch(() => {
      ...
      if (effect() === 'fade' || effect() === 'card') {
        setStore(
          'prevKey',
          store.data[(views().length + index - 1) % views().length].id
        )
  ...
  const onPrev = () => {
    batch(() => {
      ...
      if (loop()) {
        setCurrent(store.activeIndex - 1)
        setActiveIndex(
          (views().length + store.activeIndex - 1) % views().length
        )
      } else {
        const lastIndex = store.activeIndex
  ...
  const onSwipe = (swipe: any) => {
    if (effect() === 'slide') {
      setListStyle({
        ...
        transform:
          dir() === 'h'
            ? `translateX(${store.startPos - swipe.distanceX()}px)`
            : `translateY(${store.startPos - swipe.distanceY()}px)`
      })
    }
    setStore(
      'currentPos',
      store.startPos - (dir() === 'h' ? swipe.distanceX() : swipe.distanceY())
    )
  }
    ...
    >
      <div classList={classList()} style={style()} ref={wrap}>
        <div classList={arrowsClasses()}>
          ...
          <Show when={arrow()}>
            <Space
              dir={dotAlign() === 'bottom' || dotAlign() === 'top' ? 'h' : 'v'}
            >
              <div
                classList={arrowClasses()}
                x-placement="left"
                onClick={onPrev}
              >
                {dir() === 'h' ? (
        ...
        <div class="cm-carousel-list" ref={list} style={listStyle()}>
          <Show when={loop()}>
            <InnerCarouselItem
              ...
              {...store.data[store.data.length - 1]}
            />
          </Show>
          <For each={store.data}>
            {(item: any, index: Accessor<number>) => {
