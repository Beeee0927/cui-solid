...
import {
  F7ExclamationmarkTriangleFill,
  F7CheckmarkAltCircleFill,
  F7XmarkCircleFill,
  F7QuestionCircleFill,
  F7InfoCircleFill
...
export function Modal(props: ModalProps) {
  ...
  const [destroyed, setDestroyed] = createSignal(
    props.destroyOnClose && !open()
  )
      ...
      >
        ...
        >
          <div classList={classList()} style={props.style}>
            ...
            >
              {props.title ? (
                <div class="cm-modal-title">{props.title}</div>
              ) : null}
            ...
            <Show when={footer}>
              ...
              >
                <Button type={okButtonType} loading={loading()} onClick={onOk}>
                  {okText}
                </Button>
                <Button type={cancleButtonType} onClick={onCancel}>
                  {cancleText}
                </Button>
              </div>
            </Show>
...
function ModalFun() {
  ...
  return {
    open(config: ModalConfig) {
      ...
      let icon = null
      icon = () =>
        createComponent(icons[config.status!], {
          class: `cm-modal-icon-${config.status}`,
          size: 24
        })
      const close = () => {
        setTimeout(() => {
            ...
            () => (
              ...
              >
                <div class="cm-modal-left">
                  <div class="cm-modal-icon">{icon()}</div>
                </div>
