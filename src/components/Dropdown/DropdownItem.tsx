...
export function DropdownItem(props: DropdownItemProps) {
  ...
  const classList = () =>
    useClassList(local, 'cm-dropdown-item', {
  ...
  return (
    <li classList={classList()} {...others} style={style()} onClick={onClick}>
      {local.icon ? (
        <span class="cm-dropdown-item-icon">{local.icon}</span>
      ) : null}
      {local.children}
      {local.arrow ? (
        <FeatherChevronRight class="cm-dropdown-item-arrow" />
      ) : null}
    </li>
  )
}
