...
export function AvatarList(props: AvatarListProps) {
  ...
  const classList = () =>
    useClassList(local, 'cm-avatar-list', {
  ...
  const gutter = () =>
    (local.gutter ?? (local.size === 'small' ? -8 : -12)) + 'px'
    ...
    <div classList={classList()} {...rest}>
          ...
          if (index() < max()) {
            return (
              <div
                ...
                style={{ 'margin-left': index() > 0 ? gutter() : 0 }}
              >
      ...
      <Show when={avatarsLength() > max()}>
        <div class="cm-avatar-list-item" style={{ 'margin-left': gutter() }}>
          <Avatar size={local.size} style={local.excessStyle}>
            +{avatarsLength() - max()}
          </Avatar>
        </div>
      </Show>
