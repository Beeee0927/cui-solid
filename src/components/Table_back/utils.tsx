...
export function initColumns(columns: ColumnProps[]) {
  ...
  if (columns) {
    ...
    if (maxFixedLeft > -1) {
      columns[maxFixedLeft]
        ? (columns[maxFixedLeft].fixedLeftLast = true)
        : void 0
    }
    if (minFixedRight < Number.MAX_VALUE) {
      columns[minFixedRight]
        ? (columns[minFixedRight].fixedRightFirst = true)
        : void 0
    }
  }
...
export function updateScrollFixed(
  ...
  if (maxFixedLeft >= 0 || minFixedRight < Number.MAX_VALUE) {
    ...
    setStore(
      'showFixedRight',
      e.target.clientWidth + e.target.scrollLeft < e.target.scrollWidth
    )
  }
}
...
export const onResizeEnd = (store: TableStore, setStore: Function) => {
  ...
  setStore(
    ...
    produce((col: ColumnProps) => {
      col.width = col.width
        ? parseFloat(col.width) + (store.posX - store.startX) + 'px'
        : undefined
    })
  )
