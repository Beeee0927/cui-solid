...
export function Value(props: ValueProps) {
  ...
  const classList = () => ({
    'cm-field-value': true,
    'cm-field-clearable':
      props.clearable && !!props.text && !!props.text.length,
  ...
  const updateFilterStyle = () => {
    ...
    Promise.resolve().then(() => {
      ...
      const maxWidth = Math.floor(
        selection?.getBoundingClientRect().width || 10
      )
      filterInput.style.width = filterInput.scrollWidth + 'px'
      filterInput.parentElement.style.width =
        Math.min(maxWidth - 20, filterInput.scrollWidth) + 'px'
    })
  }
  ...
  return (
    <div classList={classList()} tabIndex="1" onClick={onValueClick}>
      <input type="hidden" />
      {/* 文字对齐辅助 */}
      <span
        style={{
      ...
      <Show when={props.prepend}>
        <div class="cm-field-prepend">{props.prepend}</div>
      </Show>
      <Switch>
        ...
        <Match when={!props.multi}>
          <div class="cm-field-text" ref={selection}>
            <Show when={!props.onlyInput}>
              {props.text ? (
                <span
                  style={{ display: filterStr() ? 'none' : 'inline-block' }}
                >
                  {props.text}
                </span>
              ) : (
      ...
      <span class="cm-field-cert">{props.icon}</span>
