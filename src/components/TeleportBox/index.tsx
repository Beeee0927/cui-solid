...
export function TeleportBox(props: TeleportBoxProps) {
  ...
  const classList = () =>
    useClassList(local, 'cm-teleport-box', {
    ...
    if (v) {
      const list = store.flatList?.filter((item) => {
        ...
        if (item.children?.length) {
          const childs = item.children?.filter((sub) =>
            (item.label as string).includes(v)
          )
  ...
  createEffect(() => {
    const val = value()
    const total = store.originList.filter(
      (item) => !item.disabled || (item.disabled && item.checked)
    ).length
  ...
  const renderSourceList = () => {
    return (
      <For each={store.leftList}>
        {(item, index) => {
          if (item.children?.length) {
            return <div class="cm-teleport-group-title">{item.label}</div>
          }
          return (
            local.renderSourceItem?.(item, (checked: boolean) => {
  ...
  return (
    <div classList={classList()} style={local.style} {...rest}>
      <section class="cm-teleport-left">
        <div class="cm-teleport-header">
          <span class="cm-teleport-header-total">
            总个数：{store.originList?.length}
          </span>
          <Show
            when={store.checkedAll}
            fallback={
              <Button
                size="small"
                theme="borderless"
...
const SourceItem = (props: any) => {
  ...
  return (
    ...
    >
      <div ref={props.ref} class="cm-teleport-group-title">
        {item.label}
      </div>
    </Show>
  )
