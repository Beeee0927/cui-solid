...
function generateOffset(
  ...
  const text =
    responsive === 'xs'
      ? `.${offsetPre}-${key}{margin-left: ${offset}%}`
      : `@media (min-width: ${minWidth}) { .${offsetPre}-${key}{margin-left: ${offset}%} }`

  createStyle(text, responsive)
}

function createRowGutterStyle(
  gutter: string,
  key: string,
  responsive: responsiveType
) {
...
function generateGutter(
  ...
  responsive = responsive ?? defaultResponsive
  const gu =
    typeof gutter === 'number' ? gutter.toFixed(2) : gutter[0].toFixed(2)
  const guValue = typeof gutter === 'number' ? gutter : gutter[0]
  const gap =
    typeof gutter === 'number' ? gutter.toFixed(2) : gutter[1].toFixed(2)
...
export function createGutter(gutter: number | number[] | GutterProps) {
  ...
  if (!Array.isArray(gutter) && typeof gutter === 'object') {
    for (const key in RESPONSIVE) {
      ...
      if (gu) {
        const gutterRowClass = generateGutter(
          gu,
          'gutter',
          key as responsiveType
        )
        if (gutterRowClass) {
