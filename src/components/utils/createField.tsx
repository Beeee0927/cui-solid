...
export default function createField<T>(
  props: any,
  field: any,
  defaultValue?: any
): Signal<T> {
  ...
  // 添加一个单值的监听，即 value={accessor()}
  const maybeSingleValue = !Array.isArray(props[field])
  if (maybeSingleValue) {
    createEffect(on(() => props[field], setValue, { defer: true }))
  }
